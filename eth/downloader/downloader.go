...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverBodies(pack.peerId, pack.transactions, pack.uncles, pack.producers, pack.voters)
		}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.Producers, result.Voters)
	}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles, result.Producers, result.Voters)
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Transaction, uncles [][]*types.Header, producers []types.Producers, voters []types.Voters) (err error) {
	return d.deliver(id, d.bodyCh, &bodyPack{id, transactions, uncles, producers, voters}, bodyInMeter, bodyDropMeter)
}
