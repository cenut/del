...
func (q *queue) ReserveBodies(p *peerConnection, count int) (*fetchRequest, bool, error) {
	isNoop := func(header *types.Header) bool {
		return header.TxHash == types.EmptyRootHash && header.UncleHash == types.EmptyUncleHash && header.ProducerHash == types.EmptyUncleHash && header.VoterHash == types.EmptyVoterHash
	}
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header, producers []types.Producers, voters []types.Voters) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		if types.DeriveSha(types.Transactions(txLists[index])) != header.TxHash ||
			...
			types.CalcProducerHash(producers[index]) != header.ProducerHash ||
			types.CalcVoterHash(voters[index]) != header.VoterHash {
			return errInvalidBody
		}
		result.Transactions = txLists[index]
		result.Uncles = uncleLists[index]
		result.Producers = producers[index]
